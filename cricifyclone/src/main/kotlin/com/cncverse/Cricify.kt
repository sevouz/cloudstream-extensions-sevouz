...
class Cricify(
    ...
    private suspend fun getWithCustomHeaders(url: String): String {
        ...
        return customHttpClient.newCall(request).execute().use { response -> response.body.string() }
    }
    ...
    private fun String.base64ToHexOrNull(): String? {
        ...
        return try {
            val normalized = raw.replace('-', '+').replace('_', '/').let { value ->
    ...
    private fun decryptContent(content: String): String {
        return try {
            if (content.startsWith("#EXTM3U") || content.startsWith("#EXTINF") || content.startsWith("#KODIPROP")) return content
    ...
    private fun getMpdStream(url: String, customHeaders: Map<String, String>): String {
        val client = OkHttpClient.Builder().addInterceptor(HeaderReplacementInterceptor(customHeaders)).build()
        val request = Request.Builder().url(url).build()
    ...
    private fun getDRMKeysFromLicenseServer(url: String, kid: String): String {
        ...
        val client = OkHttpClient.Builder()
            .addInterceptor(HeaderReplacementInterceptor(mapOf("User-Agent" to userAgent, "Content-Type" to "application/json;charset=UTF-8")))
        ...
        return client.newCall(request).execute().use { response ->
            ...
            val jsonResponse = parseJson<Map<String, Any>>(responseText)
    ...
    data class LoadData(
        ...
        val licenseUrl: String, val drmKeys: Map<String, String> = emptyMap(),
    ...
    override suspend fun loadLinks(
        ...
        if (loadData.url.contains("mpd")) {
            ...
            if (loadData.userAgent.isNotEmpty()) headers["User-Agent"] = loadData.userAgent
            if (loadData.cookie.isNotEmpty()) headers["Cookie"] = loadData.cookie
            ...
            if (hasValidKeys) {
                ...
                if (loadData.drmKeys.isNotEmpty()) {
                    ...
                    val mpdKidHex = regex.find(mpdStr)?.groups?.get(1)?.value?.replace("-", "")?.lowercase()
                    if (!mpdKidHex.isNullOrEmpty()) {
                        val mappedKey = loadData.drmKeys[mpdKidHex]
                        if (!mappedKey.isNullOrEmpty()) { normalizedKid = mpdKidHex; normalizedKey = mappedKey }
                    }
                }
                ...
                callback.invoke(newDrmExtractorLink(this.name, this.name, loadData.url, INFER_TYPE, CLEARKEY_UUID) {
                    this.quality = Qualities.Unknown.value
                    if (headers.isNotEmpty()) this.headers = headers
                    this.key = playerKey; this.kid = playerKid
                })
            } else if (hasLicenseUrl) {
                val mpdStr = getMpdStream(url = loadData.url, customHeaders = headers)
                val regex = Regex("""cenc:default_KID=["']([0-9a-fA-F\-]{36})["']""")
                val drmKid = regex.find(mpdStr)?.groups?.get(1)?.value ?: UUID.randomUUID().toString()
                val drmKidBytes = drmKid.replace("-", "").chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                val drmKidBase64 = Base64.encodeToString(drmKidBytes, Base64.URL_SAFE or Base64.NO_PADDING or Base64.NO_WRAP)
                val keyBase64 = getDRMKeysFromLicenseServer(url = loadData.licenseUrl, kid = drmKidBase64)
                if (keyBase64.isNotEmpty()) {
                    callback.invoke(newDrmExtractorLink(this.name, this.name, loadData.url, INFER_TYPE, CLEARKEY_UUID) {
                        this.quality = Qualities.Unknown.value
                        if (headers.isNotEmpty()) this.headers = headers
                        this.key = keyBase64.trim(); this.kid = drmKidBase64.trim()
                    }); return true
                }
                callback.invoke(newDrmExtractorLink(this.name, this.name, loadData.url, INFER_TYPE, CLEARKEY_UUID) {
                    this.quality = Qualities.Unknown.value
                ...
                callback.invoke(newExtractorLink(this.name, this.name, url = loadData.url, ExtractorLinkType.DASH) {
                    this.referer = ""; this.quality = Qualities.Unknown.value
                    if (headers.isNotEmpty()) this.headers = headers
                })
            }
            ...
            if (loadData.userAgent.isNotEmpty()) headers["User-Agent"] = loadData.userAgent
            if (loadData.cookie.isNotEmpty()) headers["Cookie"] = loadData.cookie
            callback.invoke(newExtractorLink(this.name, this.name, url = loadData.url, ExtractorLinkType.M3U8) {
                this.referer = ""; this.quality = Qualities.Unknown.value
                if (headers.isNotEmpty()) this.headers = headers
            })
            ...
            if (loadData.cookie.isNotEmpty()) headers["Cookie"] = loadData.cookie
            callback.invoke(newExtractorLink(this.name, this.name, url = loadData.url, ExtractorLinkType.M3U8) {
                this.referer = ""; this.quality = Qualities.Unknown.value; this.headers = headers
            })
        } else {
            val headers = mutableMapOf<String, String>()
            headers.putAll(loadData.headers)
            if (loadData.userAgent.isNotEmpty()) headers["User-Agent"] = loadData.userAgent
            if (loadData.cookie.isNotEmpty()) headers["Cookie"] = loadData.cookie
            callback.invoke(newExtractorLink(this.name, loadData.title, url = loadData.url, INFER_TYPE) {
                this.referer = ""; this.quality = Qualities.Unknown.value
                if (headers.isNotEmpty()) this.headers = headers
            })
        }
...
data class PlaylistItem(
    val title: String? = null, val attributes: Map<String, String> = emptyMap(),
    val headers: Map<String, String> = emptyMap(), val url: String? = null,
    val userAgent: String? = null, val key: String? = null, val keyid: String? = null,
    val cookie: String? = null, val licenseUrl: String? = null,
...
class IptvPlaylistParser {
    private fun String.hexOrNull(): String? {
        ...
        return if (normalizedHex.matches(Regex("^[0-9a-fA-F]+$"))) normalizedHex.lowercase() else null
    }

    private fun String.base64ToHexOrNull(): String? {
        val normalized = trim().replace('-', '+').replace('_', '/').let { value ->
            val padding = (4 - (value.length % 4)) % 4; value + "=".repeat(padding)
        }
        return try {
            val decoded = Base64.decode(normalized, Base64.DEFAULT)
            decoded.joinToString(separator = "") { byte -> "%02x".format(byte) }
    ...
    private fun parseLicenseKeysMap(licenseKey: String): Map<String, String> {
        ...
        return try {
            ...
            for (index in 0 until keys.length()) {
                ...
                if (!kid.isNullOrEmpty() && !key.isNullOrEmpty()) parsed[kid] = key
            }
            parsed
        ...
        if (trimmedKey.startsWith("{")) {
            return try {
                ...
                for (index in 0 until keys.length()) {
                    ...
                    val kid = item.optString("kid").normalizeDrmHexOrNull()
                    val key = item.optString("k").normalizeDrmHexOrNull()
                    if (kid != null || key != null) return key to kid
                }
                null
            } catch (_: Exception) { null }
        }
            ...
            if (line.isNotEmpty()) {
                when {
                    line.startsWith(Cricify.EXT_INF) -> {
                        bufferedTitle = line.getTitle(); bufferedAttributes = line.getAttributes()
                    ...
                    line.startsWith("#EXTHTTP:") -> {
                        ...
                        try {
                            ...
                            if (map.containsKey("cookie")) bufferedCookie = map["cookie"]
                            if (map.containsKey("user-agent")) bufferedUserAgent = map["user-agent"]
                        } catch (e: Exception) {}
                    }
                    line.startsWith(Cricify.EXT_VLC_OPT) -> {
                        ...
                        if (referrer != null) bufferedHeaders = bufferedHeaders + mapOf("Referer" to referrer)
                    }
                    line.startsWith("#KODIPROP:inputstream.adaptive.license_key=") -> {
                        ...
                        if (licenseKey.startsWith("http://") || licenseKey.startsWith("https://")) {
                            ...
                            val drmKidBytes = parts.getOrNull(0)?.replace("-", "")?.chunked(2)
                                ?.mapNotNull { try { it.toInt(16).toByte() } catch (_: NumberFormatException) { null } }?.toByteArray()
                            val drmKeyBytes = parts.getOrNull(1)?.replace("-", "")?.chunked(2)
                                ?.mapNotNull { try { it.toInt(16).toByte() } catch (_: NumberFormatException) { null } }?.toByteArray()
                            val drmKidBase64 = if (drmKidBytes != null && drmKidBytes.isNotEmpty())
                                Base64.encodeToString(drmKidBytes, Base64.URL_SAFE or Base64.NO_PADDING or Base64.NO_WRAP) else null
                            val drmKeyBase64 = if (drmKeyBytes != null && drmKeyBytes.isNotEmpty())
                                Base64.encodeToString(drmKeyBytes, Base64.URL_SAFE or Base64.NO_PADDING or Base64.NO_WRAP) else null
                            if (drmKeyBase64 != null) bufferedKey = drmKeyBase64
                            if (drmKidBase64 != null) bufferedKeyId = drmKidBase64
                        }
                    }
                    !line.startsWith("#") -> {
                        var fullLine = line; var j = i + 1
                        while (j < allLines.size && !allLines[j].trim().startsWith("#") && allLines[j].trim().isNotEmpty()) {
                            fullLine += allLines[j].trim(); j++
                        }
                        i = j - 1
                        ...
                        if (resolvedReferrer != null) finalHeaders = finalHeaders + mapOf("Referer" to resolvedReferrer)
                        if (urlOrigin != null) finalHeaders = finalHeaders + mapOf("Origin" to urlOrigin)
                        playlistItems.add(PlaylistItem(
                            title = bufferedTitle ?: "Unknown Channel", attributes = bufferedAttributes, url = url,
                            headers = finalHeaders, userAgent = urlUserAgent ?: bufferedUserAgent,
                            cookie = urlCookie ?: bufferedCookie, key = urlKey ?: bufferedKey,
                            keyid = urlKeyid ?: bufferedKeyId, licenseUrl = urlLicenseUrl ?: bufferedLicenseUrl,
                            drmKeys = bufferedDrmKeys
                        ))
                        bufferedCookie = null; bufferedUserAgent = null; bufferedHeaders = emptyMap()
                        bufferedKey = null; bufferedKeyId = null; bufferedLicenseUrl = null
                        bufferedDrmKeys = emptyMap(); bufferedTitle = null; bufferedAttributes = emptyMap()
                    }
                }
            }
            i++
    ...
    private fun String.getTitle(): String? {
        ...
        var lastCommaIndex = -1; var insideQuotes = false
        for (i in afterExtInf.indices) {
            when (afterExtInf[i]) { '"' -> insideQuotes = !insideQuotes; ',' -> if (!insideQuotes) lastCommaIndex = i }
        }
        ...
        else afterExtInf.split(",").lastOrNull()?.replaceQuotesAndTrim()
    }
    ...
    private fun String.getUrlParameter(key: String): String? {
        ...
        for (param in paramsString.split("&")) {
            ...
            if (keyValuePair.size == 2 && keyValuePair[0].trim().equals(key, ignoreCase = true))
                return keyValuePair[1].trim().replaceQuotesAndTrim()
        }
        return null
    }
    ...
    private fun String.getAttributes(): Map<String, String> {
        ...
        val attributesString = if (lastCommaIndex != -1) afterExtInf.substring(0, lastCommaIndex).trim() else afterExtInf.trim()
        ...
        attributeRegex.findAll(attributesString).forEach { matchResult ->
            val k = matchResult.groups[1]?.value ?: ""
            val v = (matchResult.groups[2]?.value ?: matchResult.groups[3]?.value ?: "").trim()
