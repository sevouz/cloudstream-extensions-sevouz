...
class AnikageProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        val lang = if (epData.isDub) "dub" else "sub"
        ...
        val providers = serversData.amap { it.id }.takeIf { it.isNotEmpty() }
            ?: listOf("megg", "miko", "anya", "verse", "neko")
            ...
            async {
                ...
                try {
                    ...
                    sourceData.sources?.amap { src ->
                        ...
                        callback(newExtractorLink("Anikage", nameStr, videoUrl,
    ...
    data class AnikageResponse(val hasNextPage: Boolean, val results: List<AnimeResult>)
    data class AnimeDetailsResponse(val anime: AnimeResult)
    data class AnimeResult(
        val slug: String, val title: AnimeTitle, val coverImage: CoverImage,
        val type: String, val format: String, val status: String,
        val genres: List<String>?, val year: Int?, val description: String?,
        val anilistId: Int?, val idMal: Int?, val bannerImage: String?
    )
    data class AnimeTitle(val romaji: String, val english: String?)
    data class CoverImage(val large: String?, val extraLarge: String?)
    data class EpisodeResult(
        val number: Int, val title: String?, val description: String?,
        val img: String?, val rating: Double?, val airDate: String?
    )
    data class EpisodeSource(
        val sources: List<SourceData>?, val subtitles: List<SubtitleData>?, val embeds: List<EmbedData>?
    )
    data class ServerData(val id: String)
    data class EmbedData(val url: String, val type: String?, val server: String?)
    data class SourceData(val url: String, val quality: String?, val isM3U8: Boolean?)
    data class SubtitleData(val file: String, val label: String?)
}
