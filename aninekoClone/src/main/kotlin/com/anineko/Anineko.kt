...
class Anineko : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (anilistId != null) {
            val aniZipResponse = app.get("https://api.ani.zip/mappings?anilist_id=$anilistId").text
        ...
        targetPanels.forEach { panel ->
            panel.select(".server-video").forEach { serverBtn ->
                ...
                if (m3u8Url != null) {
                    ...
                    generateM3u8(sourceName, m3u8Url, finalUrl).forEach(callback)
                    ...
                    extractor.getUrl(finalUrl, "$mainUrl/", subtitleCallback) { link -> links.add(link) }
                    links.forEach { link ->
                        callback.invoke(
                            newExtractorLink(
                                source = link.source,
                    ...
                    val links = mutableListOf<ExtractorLink>()
                    loadExtractor(finalUrl, "$mainUrl/", subtitleCallback) { link -> links.add(link) }
                    links.forEach { link ->
                        callback.invoke(
                            newExtractorLink(
                                source = link.source,
